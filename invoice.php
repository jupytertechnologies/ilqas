...
if(isset($_GET['user_id'])){
	...
	if(is_null($datas)){//If there is no laboratory details in the lab_details table for this user's lab, redirect back to dashboard
		echo"<script>alert('To get your invoice, Update laboratory details under Settings Menu')</script>";
		echo"<script>window.open('dashboard-3.php?user= $user_id','_self')</script>";
	}else{//If the laboratory exist in the lab_details table, open the invoice
	$cart_items = $objRequest->get_cart_items($user_id);
	if(is_null($cart_items)){
		echo"<script>alert('You have no items in the cart. Complete request and add parameters')</script>";
		echo"<script>window.open('make_request.php?user_id= $user_id','_self')</script>";
	}elseif(!is_null($cart_items)){
                    ...
                    <div class="container mt-4">
                        ...
                        <div class="card invoice">
                            ...
                            <div class="card-body p-4 p-md-5">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-borderless mb-0">
                                        ...
                                        <tbody>
											<?php
											//Create object to get cart items to populate the invoice
											$cart_items = $objRequest->get_cart_items($user_id);
											$subtotal = $objRequest->get_cart_subtotal($user_id);
											$cart_item_no = $objRequest->count_cart_items($user_id);
											$count = intval($cart_item_no);
											$vat = $subtotal*0.18;
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right pb-0"><div class="h5 mb-0 font-weight-700"><?php echo number_format($subtotal); ?> </div></td>
                                            </tr>
                                            <!-- Invoice tax column-->
                                            <tr>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right pb-0"><div class="h5 mb-0 font-weight-700 text-green"><?php echo number_format($total_amount_due); ?></div></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
							...
							<div class="d-flex justify-content-between">
								<a class="btn btn-secondary rounded-pill px-4 mr-8 my-1" href="make_request.php?user_id=<?php echo$user_id; ?>">Back</a>
